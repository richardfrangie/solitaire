...
function canMoveSelectedTo(target) {
  ...
  if (isFoundationClicked)
    return isCardClicked
      ? haveSameSuit(comparisonCard, target) && isOneRankAbove(comparisonCard, target)
    ...
    return isCardClicked
      ? haveDifferentColors(comparisonCard, target) && isOneRankBelow(comparisonCard, target)
...
function isGameWon() {
  return (
    foundations[0].children.length == 13 &&
...
function handleCardsClick(event) {
  ...
  if (game.isSelectionActive())
    return isCardInSelection(target) ? deselectCards() : moveSelectedTo(target);
  else if (isCardUpClicked) return selectCardsFrom(target);
}
