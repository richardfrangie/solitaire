...
and a single Web Component (~playing-card~). There is no framework, no dependencies, and no test suite: game logic is plain DOM manipulation in one file.
...
- Custom Elements :: Playing card web component
